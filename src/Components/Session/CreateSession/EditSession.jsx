...
const EditSession = () => {
  ...
  useEffect(() => {
    ...
    const newOptions2 = sessionTypeOptions.map((option) => {
      if (option.value === session.sessionInfo.sessionFor[0].value) {
        return { ...option, checked: true };
      } else {
    ...
    const newOptions3 = sessionRoleOptions.map((option) => {
      if (option.value === session.sessionInfo.sessionForRole[0].value) {
        return { ...option, checked: true };
      } else {
        return { ...option, checked: false };
      }
